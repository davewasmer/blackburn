...
export default Renderer.extend({
    ...
    if (isArray(payload)) {
      if (payload[0] instanceof Error) {
        ...
        document.data = payload.map((record) => {
          this.renderRecord(record, document, options);
        });
      }
    } else {
      if (payload instanceof Error) {
        document.errors = [ this.renderError(options, payload) ];
      } else {
        document.data = this.renderRecord(options, document, payload);
      }
    }
  },
  ...
  renderIncluded(payload, document, options) {
    if (options.included) {
      ...
      document.included = options.included.map(this.renderRecord.bind(this, options));
    }
  },
  ...
  attributesForRecord(record, options) {
    return this.serializerFor(record).serializeAttributes(record, options);
  },
   ...
   *                                         relationship
   */
  linksForRelationship(name, descriptor, record) {
    let recordURL = this.linksForRecord(record).self;
    return {
   ...
   * @param  {String[]|Number[]}    descriptor.ids  array of related ids (hasMany)
   * @param  {String|Number}        descriptor.id   related record id (hasOne)
   * @param  {Object}               descriptor.record   related record instance (hasOne)
   * @param  {Object}               descriptor.records  related record instances (hasMany)
  ...
  dataForRelationship(name, descriptor, record, document, options) {
    if (descriptor.records) {
      descriptor.ids = [];
      descriptor.records.map((related) => {
        this.includeRecord(related, document, options);
        descriptor.ids.push(this.adapter.idForRecord(related, options));
      });
    } else if (descriptor.record) {
      this.includeRecord(record, document, options);
      descriptor.id = this.adapter.idForRecord(record, options);
    }
    if (descriptor.ids) {
      return descriptor.ids.map((id) => {
        return {
  ...
  linksForRecord(record) {
    return {
      self: '/' + this.adapter.typeForRecord(record) + '/' + this.adapter.idForRecord(record)
    };
  },
   ...
   * @param  {Object}      options
   */
  includeRecord(record, document, options) {
    if (!isArray(document.included)) {
      document.included = [];
    }
    document.included.push(this.renderRecord(record, document, options));
  },
